...
namespace zero_mate::gui
{
    ...
    bool CSource_Code_Window::Highlight_Code_Block(const utils::elf::TSource_Code& source_code,
    ...
    void CSource_Code_Window::Render_Code_Block(const utils::elf::TSource_Code& source_code,
                                                std::size_t& idx)
    {
        ...
        if (ImGui::CollapsingHeader(
            fmt::format("{}##{}", source_code.code[idx].disassembly, source_code.code[idx].index).c_str()))
        {
                                  ...
                                  Table_Flags))
            {
                ...
                while (idx != source_code.code.size())
                {
                    ...
                    if (ImGui::RadioButton(fmt::format("##{}", source_code.code[idx].addr).c_str(),
                                           m_breakpoints[source_code.code[idx].addr]) &&
                        !m_cpu_running)
                    {
