...
TEST(cmp_instruction, test_01)
{
    ...
    EXPECT_EQ(register_state_checker.Is_Any_Other_Register_Modified(
              cpu.Get_CPU_Context(), { { .idx = 0, .expected_value = 186 },
...
TEST(cmp_instruction, test_02)
{
    ...
    // clang-format off
    EXPECT_EQ(register_state_checker.Is_Any_Other_Register_Modified(
              cpu.Get_CPU_Context(), { { .idx = 0, .expected_value = 0xFFFFFFFF },
...
TEST(cmp_instruction, test_03)
{
    ...
    // clang-format off
    EXPECT_EQ(register_state_checker.Is_Any_Other_Register_Modified(
              cpu.Get_CPU_Context(), { { .idx = 0, .expected_value = 0x0 },
                                       { .idx = 1, .expected_value = 0xFFFFFFFF } }), false);
    // clang-format on
...
TEST(cmp_instruction, test_05)
{
    ...
    // clang-format off
    EXPECT_EQ(register_state_checker.Is_Any_Other_Register_Modified(
              cpu.Get_CPU_Context(), { { .idx = 0, .expected_value = 0 } }), false);
    // clang-format on
